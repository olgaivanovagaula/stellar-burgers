...
export const createBurgerSlice = createSlice({
  ...
  reducers: {
    ...
    // upItem: (state, action) => {
    //     const index = state.ingridients.findIndex((item) => item.id === action.payload.id)

    //     if (index > 0) {
    //         [state.ingridients[index-1], state.ingridients[index]] = [state.ingridients[index], state.ingridients[index-1]]
    //     }
    // },
